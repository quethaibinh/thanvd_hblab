...
import com.example.demo.infrastructure.adapter.out.persistence.jpa.adapter.JpaNewResourceAdapter;
...
public class DemoDataInitializer implements CommandLineRunner {
        ...
        private final JpaNewResourceAdapter newResourceAdapter;
        ...
        public DemoDataInitializer(
                        ...
                        Clock clock,
                        JpaNewResourceAdapter newResourceAdapter) {
                ...
                this.clock = clock;
                this.newResourceAdapter = newResourceAdapter;
        }
        ...
        public void run(String... args) {
                ...
                reactionRepository.save(new ArticleReaction("reaction-1", articleOne.id(), alice.id(),
                                ReactionType.LOVE, now.minus(5, ChronoUnit.HOURS)));
                reactionRepository.save(new ArticleReaction("reaction-2", articleOne.id(), bob.id(), ReactionType.LIKE,
                                now.minus(4, ChronoUnit.HOURS)));
                reactionRepository.save(new ArticleReaction("reaction-3", articleTwo.id(), alice.id(), ReactionType.WOW,
                                now.minus(14, ChronoUnit.HOURS)));

                // ── Seed crawler news sources ─────────────────────────────────────────────
                seedNewResourceIfAbsent("src-vnexpress",  "VnExpress",   "vnexpress.net",   "https://vnexpress.net/rss",         9.5);
                seedNewResourceIfAbsent("src-thanhnien",  "Thanh Nien",  "thanhnien.vn",    "https://thanhnien.vn/rss",          9.0);
                seedNewResourceIfAbsent("src-techcrunch", "Tech Crunch", "techcrunch.com",  "https://techcrunch.com/feed",       8.7);
        }

        private void seedNewResourceIfAbsent(String id, String name, String domain, String rssUrl, double score) {
                if (!newResourceAdapter.existsByDomain(domain)) {
                        newResourceAdapter.save(new NewResource(id, name, domain, rssUrl, score, "ACTIVE"));
                }
        }
}
